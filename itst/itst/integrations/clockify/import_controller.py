...
class BaseImportController(ABC):
    """
    Abstract base class that defines all methods as abstract.
    The docstrings and signatures are kept exactly as in your original code.
    """

    @abstractmethod
    def validate_project_existence(self, project_name: str) -> bool:
        """
    ...
    @abstractmethod
    def _calculate_times(self, entry: Dict) -> Dict[str, float]:
        """
...
class ImportController(BaseImportController):
    """
    Concrete class implementing all abstract methods from BaseImportController.
    The docstrings and logic remain unchanged.
    """
    def validate_project_existence(self, project_name: str) -> bool:
    ...
    def build_timesheet_detail_data(
        ...
        time_data = self._calculate_times(entry)
        
        project_name = entry["project"]["name"]
        entry_id = entry["id"]

        billing_rate = entry["hourlyRate"]["amount"] / 100
        biling_amount = billing_rate * time_data["duration_hours"]
    ...
    def process_clockify_entry_to_erpnext(
        ...
        timesheet_detail_data = self.build_timesheet_detail_data(
            entry, 
    ...
    def import_clockify_entries_to_timesheet(
        ...
        for entry in entries:
            project_name = entry ["project"]["name"]
            try:
                if not self.validate_project_existence(project_name):
                ...
                result = self.process_clockify_entry_to_erpnext(
                    entry,
                    employee_id, 
                    employee_name,
                    activity_type,
